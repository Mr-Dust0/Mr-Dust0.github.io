...
published: true
categories: ["Crypto"]
tags: ["sha",  "git", "CVE-2022-44268", "CVE-2022-4510"]
---
# Hash extention
...
## How it works
As the hash that is outputted is just the internal state of the hash function when it does not need to compress anymore data we know the internal state of the hash function is the internal state that will be used if a new block is added. 
...
compresses has changed beacuse it is not in a new block. The padding that is used on most algorithms is the text in the last block that isn't a multiple of 64 is padded until it is 8 less than a full 64 byte block.
